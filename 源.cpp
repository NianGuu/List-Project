...
head:
	...
	choose = ToInt32(gets_s(chooseChar));
	switch (choose) {
	case 1:UI_AttributesShop(*player);break;
	case 2:UI_EquiShop(*player);break;
	case 3:UI_FoodShop(*player);break;
	case 0:return 0;
	default: {
		printf("\n请输入正确的选项！\n");
		Sleep(300);
...
Weapon CatchWeapon(int i) {
	...
	Weapon weapon_One{ 1,"博弈失败掏出白太",10,1000 };
	Weapon weapon_Two{ 2,"我振他惊雷啊",20,2000 };
	Weapon weapon_Three{ 3,"好振下一把",30,3000 };
	/*武器返回*/
	switch (i) {
...
Armour CatchArmour(int i) {
	...
	Armour armour_One{ 1,"孙悟空的戏法",10,1000 };
	Armour armour_Two{ 2,"尹娜的幻身",20 ,2000 };
	Armour armour_Three{ 3,"御宅花织",30,3000 };
	/*盔甲返回*/
	switch (i) {
