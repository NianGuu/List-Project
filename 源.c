...
head:
	...
	switch (i) {
	case 1:strcpy_s(iname, sizeof(iname), "小怪");break;
	case 2:strcpy_s(iname, sizeof(iname), "精英");break;
	case 3:strcpy_s(iname, sizeof(iname), "首领");break;
	}
	printf("你遇到了一个%s！\n", iname);
	printf("*****************\n");
	Entity mob;
	...
	printf("Y/N\n");
	char ch;
	while (ch = getchar()) {					//让玩家选择是否战斗，是则继续，否则跳转至子函数开头
		while (getchar() != '\n');
		if (ch == 'Y' || ch == 'y')
			break;
		else if (ch == 'N' || ch == 'n')
